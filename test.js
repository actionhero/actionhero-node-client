...
suite.addBatch({
  "I can run an action (simple)": {
    ...
    'resp (simple)?' : function(res, apiResposne){ 
      ...
      assert.strictEqual(apiResposne.uptime > 0, true);
      assert.strictEqual(apiResposne.stats['connections:toatalActiveConnections'] == 1, true);
    }
  }
});
...
suite.addBatch({
  "I get events for say": {
    topic: function(){
      ...
      api.chatRoom.socketRoomBroadcast(null, "TEST MESSAGE");
    },
